// controllers/tool_list_controller.dart (ĐÃ CẬP NHẬT)
...
class ToolListController extends GetxController {
  ...
  Future<void> fetchStoreProducts() async {
    try {
      ...
      // 4. Lọc danh sách sản phẩm lần đầu (chỉ hiển thị 'tools')
      filterProducts();
  ...
  void _fetchBrandsFromCache() {
    // Lấy 'brand' từ các sản phẩm có category là 'tools'
    final fetchedBrands = _allStoreProducts
        .where((p) => p.product.category == 'tools')
  ...
  void filterProducts() {
    ...
    var filteredList = _allStoreProducts.where((storeProduct) {
      ...
      final bool isCategoryMatch = storeProduct.product.category == 'tools';
