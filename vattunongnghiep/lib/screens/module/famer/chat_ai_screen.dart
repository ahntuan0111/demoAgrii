import 'package:agri_flutter/models/message.dart';
import 'package:agri_flutter/screens/module/famer/camera_screen.dart';
import 'package:agri_flutter/screens/module/famer/my_suggestions_page.dart';
import 'package:agri_flutter/screens/module/famer/suggestions_page.dart';
import 'package:agri_flutter/screens/module/famer/voice_recorder_screen.dart';
import 'package:agri_flutter/services/chat_history_service.dart';

import 'package:agri_flutter/services/chatbox_ai_service.dart';
import 'package:agri_flutter/shared/widgets/chat_input.dart';
...
class ChatPage extends StatefulWidget {
  final String? initialQuestion; // câu hỏi được truyền từ màn hình trước
...
class _ChatPageState extends State<ChatPage> with TickerProviderStateMixin {
  ...
  Future<void> _checkServerHealth() async {
    try {
      ...
      // Even if health check fails, we should still try to connect
      // This prevents false negatives when the health endpoint is not available
      setState(() {
        _serverConnected =
            true; // Assume connected and let actual requests fail if needed
      });
    }
  }
  ...
  void _sendMessage() async {
    ...
    try {
      ...
      if (_pendingFile != null) {
        ...
        print('Sending chat message: $prompt');
      ...
      if (e.toString().contains('SocketException') ||
        ...
        errorMsg = 'Lỗi: Server trả về lỗi. Vui lòng kiểm tra log server.';
      } else if (e.toString().contains('AI Service Error')) {
        // More specific AI service error
        errorMsg = 'Lỗi từ AI Service: ${e.toString()}';
      } else if (e.toString().contains('Failed to parse response')) {
        errorMsg =
            'Lỗi: Không thể phân tích phản hồi từ server. Vui lòng kiểm tra log server.';
      } else {
        // For all other errors, show a more generic message that doesn't assume connection issues
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text(
              'Có lỗi xảy ra khi xử lý file. Kiểm tra console để biết chi tiết.',
            ),
            backgroundColor: Colors.red,
            duration: const Duration(seconds: 5),
            action: SnackBarAction(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // Nút xem đề xuất luôn hiển thị theo yêu cầu của project specification
            if (!message.isUser)
              Padding(
                padding: const EdgeInsets.only(top: 8),
                child: ElevatedButton.icon(
                  onPressed: () {
                    // Kiểm tra nếu có dữ liệu thực sự để hiển thị
                    bool hasData =
                        (message.csvResults != null &&
                            message.csvResults!.isNotEmpty) ||
                        (message.suggestions != null &&
                            message.suggestions!.isNotEmpty) ||
                        (message.keywords != null &&
                            _containsMeaningfulKeyword(message.keywords!));
                    ...
                    if (hasData) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Không có đề xuất nào'),
                          backgroundColor: Colors.orange,
                        ),
                      );
                    }
  ...
  /// Kiểm tra xem một message có đủ dữ liệu để hiển thị nút "Xem đề xuất"
  /// Kiểm tra message có đủ dữ liệu để hiển thị nút "Xem đề xuất"
  bool _hasRelevantKeywords(Message message) {
    // 1. CSV results luôn được coi là dữ liệu gợi ý
    if (message.csvResults != null && message.csvResults!.isNotEmpty) {
      return true;
    }

    // 2. Keywords/Suggestions: chỉ tính các giá trị meaningful (không phải greetings)
    if ((message.keywords != null &&
            _containsMeaningfulKeyword(message.keywords!)) ||
        (message.suggestions != null &&
            _containsMeaningfulKeyword(message.suggestions!))) {
      return true;
    }

    // Không có dữ liệu gợi ý → không hiện nút
    return false;
  }

  /// Hàm đệ quy kiểm tra Map/List/String chứa keyword thực sự (không phải greeting)
  bool _containsMeaningfulKeyword(dynamic data) {
    ...
    if (data is String) {
      ...
      if (_isGenericGreeting(trimmed)) return false; // loại bỏ greetings
      return true;
    }
  ...
  /// Helper loại bỏ các greetings/câu chào phổ biến
  bool _isGenericGreeting(String value) {
    final greetings = [
      'xin chào',
      'hello',
      'hi',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        children: [
          // Show connection warning if server is not connected
          if (!_serverConnected)
            Container(
              ...
              child: const Row(
                children: [
                  ...
                  SizedBox(width: 8),
                  Text(
                    'Server không phản hồi. Kiểm tra kết nối.',
