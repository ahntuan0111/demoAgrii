...
import '../themes/image_helper copy.dart';
      ...
      children: [
        ...
        Expanded(
          child: TabBarView(
            ...
            children: [
              if (widget.suggestionData != null)
                _ProductSuggestionsTabNew(
                  products: widget.suggestionData!.products,
                )
              else
                _ProductSuggestionsTab(csvResults: widget.csvResults),
              if (widget.suggestionData != null)
                _StoresTabNew(stores: widget.suggestionData!.stores)
              else
                _StoresTab(csvResults: widget.csvResults),
              if (widget.suggestionData != null)
                _ExpertsTabNew(farmers: widget.suggestionData!.farmers)
              else
...
/// Product suggestions tab using new data structure
class _ProductSuggestionsTabNew extends StatelessWidget {
  final List<dynamic> products;

  const _ProductSuggestionsTabNew({required this.products});
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return _ProductCardNew(product: products[index]);
      },
    );
  }
}

/// Product card widget with enhanced disease image display using new data structure
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      Text(
                        'Công dụng: ${product['usage'] ?? 'Không có thông tin'}',
                        style: TextStyle(fontSize: 13, color: Colors.grey[600]),
                      ),
                    ],
                  ),
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                            'Đã thêm "${product['name'] ?? 'sản phẩm'}" vào giỏ hàng',
                          ),
                          backgroundColor: Colors.green,
                        ),
                      );
...
/// Stores tab using new data structure
class _StoresTabNew extends StatefulWidget {
  final List<dynamic> stores;

  const _StoresTabNew({required this.stores});

  @override
  _StoresTabNewState createState() => _StoresTabNewState();
}

class _StoresTabNewState extends State<_StoresTabNew> {
  @override
  Widget build(BuildContext context) {
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              ...
                              const SizedBox(width: 4),
                              Text(
                                store['hours'] ?? 'Không có giờ mở cửa',
                                style: TextStyle(
                                  fontSize: 14,
                                  color: Colors.grey[600],
                                ),
                              ...
                              Container(
                                ...
                                child: Text(
                                  store['distance'] ?? 'Không có khoảng cách',
                                  style: TextStyle(
                                    fontSize: 12,
                ...
                Row(
                  children: [
                    ...
                    ElevatedButton.icon(
                      ...
                      icon: const Icon(Icons.shopping_cart, size: 18),
                      label: const Text('Đặt tại cửa hàng'),
                      style: ElevatedButton.styleFrom(
                        backgroundColor: Colors.orange[600],
                        foregroundColor: Colors.white,
                        padding: const EdgeInsets.symmetric(
...
/// Experts tab using new data structure
class _ExpertsTabNew extends StatefulWidget {
  final List<dynamic> farmers;

  const _ExpertsTabNew({required this.farmers});

  @override
  _ExpertsTabNewState createState() => _ExpertsTabNewState();
}

class _ExpertsTabNewState extends State<_ExpertsTabNew> {
  @override
  Widget build(BuildContext context) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        final farmer = widget.farmers[index];
        return Card(
          margin: const EdgeInsets.only(bottom: 16),
          elevation: 2,
          shape: RoundedRectangleBorder(
                ...
                Row(
                  children: [
                    ...
                    if (farmer['image'] != null &&
                        farmer['image'].toString().isNotEmpty)
                      ImageHelper.buildImageWidget(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              ...
                              const SizedBox(width: 4),
                              Text(
                                farmer['distance'] ?? 'Không có khoảng cách',
                                style: TextStyle(
                                  fontSize: 14,
                                  color: Colors.grey[600],
                                ),
                          ...
                          Row(
                            children: [
                              ...
                              const SizedBox(width: 4),
                              Text(
                                '${farmer['specialty'] ?? 'Không có chuyên môn'} - ${farmer['experience'] ?? 'Không có kinh nghiệm'}',
                                style: TextStyle(
                                  fontSize: 14,
                                  color: Colors.grey[600],
                                ),
                    ...
                    OutlinedButton.icon(
                      onPressed: () {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              'Đang mở chat với ${farmer['name'] ?? 'lão nông'}...',
                            ),
                            backgroundColor: Colors.orange,
                          ),
                        );
                    ...
                    ElevatedButton.icon(
                      onPressed: () {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              'Đặt lịch với ${farmer['name'] ?? 'lão nông'}',
                            ),
                            backgroundColor: Colors.purple,
                          ),
                        );
